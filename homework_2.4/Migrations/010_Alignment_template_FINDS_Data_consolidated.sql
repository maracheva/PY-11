...
set @TSQL = 'insert into #@tmp_MSNRANGE select *  from openquery(R_PSS_A400M,''
SELECT
...
''select
	...
	set 
		color = '' 
	from  
		#@Result join (select [FIN DS number] from #@Result group by [FIN DS number] having COUNT(*) = 1) as ROUGE on ROUGE.[FIN DS number] = #@Result.[FIN DS number]
