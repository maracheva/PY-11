...
-- Create the login only if it does not already exists (re-entrance)
IF ((SELECT count(Name) FROM master.dbo.syslogins WHERE loginname = 'SADM' and isntname = 0) = 0)
	-- Add the login
	exec sp_addlogin 'SADM', 'SADM', @PSE_Database
GO

-- Create the user
IF ((select count(Name) from sysusers  WHERE [name]='SADM' and isntname = 0) = 0)
	-- Add the login
	EXEC sp_adduser 'SADM', 'SADM'
GO

--EXEC sp_change_users_login 'Update_One', 'SADM', 'SADM'

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[T_SIRD]') and OBJECTPROPERTY(id, N'IsTable') = 1)
	GRANT SELECT ON T_SIRD TO SADM

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[T_SWRD]') and OBJECTPROPERTY(id, N'IsTable') = 1)
	GRANT SELECT ON T_SWRD TO SADM
